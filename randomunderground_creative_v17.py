import os, io, re, math, random, asyncio, subprocess, tempfile, shutil, textwrap
...
JJ_PRESETS = {
    'cute': ('💗 Cute Beat', 0.62),
    'flash': ('✨ Pink Flash', 0.48),
    'dark': ('🖤 Dark Beat', 0.58),
    'y2k': ('💿 Y2K', 0.52),
    'cinematic': ('🎞️ Cinematic', 0.82),
...
def creative_menu():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton('🖼️ PHOTO TOOLS', callback_data='creative:photo')],
...
def photo_menu():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton('📸 POLAROID', callback_data='photo:polaroid'), InlineKeyboardButton('🎞️ PHOTO DUMP', callback_data='photo:dump')],
...
async def creative_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        '🩷 WANTBASE CREATIVE\n\nBikin JJ, edit foto, main game, atau cari ide caption. Pilih aja:',
        reply_markup=creative_menu())

async def jj_command(update, context):
    context.user_data['creative_mode'] = 'jj_choose'
    await update.message.reply_text('🎬 WANTBASE JJ MAKER\n\nPilih style yang mau dipakai:', reply_markup=jj_menu())

async def creative_callback(update, context):
    q=update.callback_query; await q.answer(); data=q.data
    if data == 'creative:home':
        context.user_data['creative_mode']=None; await q.edit_message_text('🩷 WANTBASE CREATIVE', reply_markup=creative_menu()); return
    if data == 'creative:jj':
    ...
    if data == 'creative:photo':
        await q.edit_message_text('🖼️ PHOTO TOOLS', reply_markup=photo_menu()); return
...
async def jjmusic_command(update, context):
    if context.user_data.get('creative_mode') != 'jj_upload':
        await update.message.reply_text('Mulai /jj dulu yaa.')
        return
...
async def done_command(update, context):
    ...
    if len(paths)<2:
        await update.message.reply_text('📸 Minimal 2 foto dulu yaa.'); return
    await update.message.reply_text('⏳ Lagi bikin JJ kamu... jangan tutup chat dulu 🩷')
    try:
        out=await asyncio.to_thread(render_jj, paths, context.user_data['jj_preset'], context.user_data.get('jj_audio'))
...
async def creative_photo_handler(update, context):
    ...
    if mode == 'jj_upload':
        ...
        if len(paths)>=MAX_PHOTOS:
            await update.message.reply_text('Maksimal 10 foto. Ketik /done yaa.'); return True
    ...
    if mode in {'polaroid','dump','sticker','meme'}:
        ...
        try:
            out=await asyncio.to_thread(render_photo_tool,path,mode,context.user_data.get('meme_text',''))
            if mode=='sticker': await update.message.reply_sticker(sticker=out.open('rb'))
            else: await update.message.reply_photo(photo=out.open('rb'), caption='✨ Made with WANTBASE')
        finally:
...
def render_photo_tool(path, mode, text=''):
    ...
    if mode=='polaroid':
        photo=_fit(im,(900,900)); canvas=Image.new('RGBA',(1000,1150),'white'); canvas.alpha_composite(photo,(50,50)); d=ImageDraw.Draw(canvas); d.text((500,1010),'WANTBASE',font=_font(38),fill='black',anchor='mm'); out=RENDER_DIR/f'{random.randrange(10**9)}.png'; canvas.save(out); return out
    if mode=='dump':
        photo=_fit(im,(1080,1080)); canvas=Image.new('RGBA',(1080,1350),(245,240,238,255)); canvas.alpha_composite(photo,(0,0)); d=ImageDraw.Draw(canvas); d.text((54,1140),'PHOTO DUMP  /  WANTBASE',font=_font(34),fill=(35,35,35),stroke_width=1); out=RENDER_DIR/f'{random.randrange(10**9)}.png'; canvas.save(out); return out
    if mode=='sticker':
...
async def dump_command(update,context): context.user_data['creative_mode']='dump'; await update.message.reply_text('🎞️ Kirim fotonya.')
...
async def image_simple(update,context,mode):
    ...
    try:
        ...
        out=RENDER_DIR/f'{random.randrange(10**9)}.jpg'; im.save(out,quality=82,optimize=True); await update.message.reply_photo(photo=out.open('rb'),caption='✨ WANTBASE PHOTO TOOL')
    finally: path.unlink(missing_ok=True); out.unlink(missing_ok=True)

# ---------- fun ----------
async def eightball(update,context):
    answers=['✨ IYA BANGET.','😭 JANGAN DULU.','👀 KAYAKNYA...','💗 GAS.','🫧 COBA TANYA LAGI.','🔥 YES.','🤡 MENDING TIDUR.']; await update.message.reply_text('🎱 '+random.choice(answers))
...
async def wyr(update,context): await update.message.reply_text('⚔️ WOULD YOU RATHER\n\nA. Punya barang incaran tapi nggak boleh beli 1 tahun\nB. Boleh checkout bebas tapi nggak boleh menyesal 😭\n\nA atau B?')

# ---------- text ----------
CAPTIONS=['just a little moment ✨','main character energy.','currently romanticizing my life.','soft days, pretty things.','proof that today happened.','no context, just vibes.']
BIOS=['♡ collecting little moments','౨ৎ online sometimes, pretty always','just here for the plot','soft heart, loud mind','made of playlists & late nights']
...
async def username(update,context):
    a=['luv','wants','bby','cloud','mori','peach','daisy','angel']; b=['archive','jpg','core','files','diary','room','era']; await update.message.reply_text('🎀 USERNAME\n\n@'+random.choice(a)+random.choice(b)+str(random.randint(0,99)))
